...
const useStore = <T extends Record<string, unknown>>(initialStore: T): [T, unknown, (path: keyof T, defaultValue: unknown) => void, React.Dispatch<React.SetStateAction<T>>, (path?: keyof T) => void] => {
	 ...
	 * @returns void
	 */
	const setStoreState = (path: keyof T, value: unknown) => {
